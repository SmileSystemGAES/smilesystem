...
public class OdontoEspeciDAO {
  PreparedStatement pstmt = null;
    ...
    public OdontoEspecDTO obtenerUno(int idusu, int idrol) {
        ...
        String sqlOne = "SELECT  idOdontologo, idEspecializacion FROM odontoespec WHERE idOdontologo = ?";
        try {
    ...
    public String modificarOdontoEspec(OdontoEspecDTO odto) {
        try {
            String sqlUpdate = "UPDATE  odontoespec SET  idEspecializacion = ? WHERE idOdontologo = ?";
    ...
    public String eliminarOdontoEspec(OdontoEspecDTO odto) {
        try {
            String sqlDelete = "DELETE FROM `smilesystemv2`.`odontoespec` WHERE `odontoespec`.`idOdontologo` = ? ";
